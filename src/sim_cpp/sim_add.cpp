...
int main(int argc, char** argv) {
		...
		FP21_add->eval();
 	  	...
 	  	if (FP21_add->clk && ((contextp->time() > ((PIPELINE_DELAY-1)*2)  ))) {
 	  		...
 	  		printf("t=%d et=%d a=%.6lf b=%.6lf expt=%.6lf rly=%.6lf \n",
 	  			...
 	  			error_count++;
 	  			printf("sign_c=%b exp_c=%b frac_c=%b\n",FP21_add->sign_c_out, FP21_add->exp_c_out, FP21_add->frac_c_out); //needs better formating
 	  		}
 	  	}
	...
	printf("error_rate: %f%\n", error_count / ((double)(SIM_STEPS - PIPELINE_DELAY - 1) / 2) * 100.0); //minor error may be here, need to look into this.
